...
| --------------------------- | -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- |
| Product name                | Decision required          | The phrase is descriptive and several active game-discovery products use materially similar names. Preliminary indexed searches did not surface a conclusive exact-match registration, but they are not trademark clearance. | Choose whether to rename, then run official searches in the intended launch jurisdictions.                 |
| Primary domain              | Blocked                    | Registry RDAP reported `whattoplaynext.com` as registered on 2024-08-27 and currently held through NameCheap.                                                                                                                | Choose the final name before purchasing a domain; recheck availability immediately before registration.    |
| Twitch application          | Owner action required      | IGDB requires Twitch application credentials; Twitch requires a verified account, 2FA, and a unique application name.                                                                                                        | Register the application after the naming decision and store its credentials outside Git.                  |
...
- [ ] Twitch application created and credentials stored privately.
